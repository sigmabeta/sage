 ...
 * warning on every build. This plugin only configures the target when the `chipbox.js` Gradle
 * property is set, so ordinary Android/JVM builds never touch Kotlin/JS and stay quiet. Web
 * builds pass `-Pchipbox.js=true`; `apps/js` requires it, since it consumes the JS variants these
 * modules then expose.
 *
...
class SageKmpJsPlugin : Plugin<Project> {
    override fun apply(target: Project) {
        with(target) {
            val jsEnabled = providers.gradleProperty("chipbox.js").orNull.toBoolean()
            if (!jsEnabled) return
