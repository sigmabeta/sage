...
class StringGenerator(private val random: Random) {
    ...
    fun generateTitle(): String = generateWords(MAX_WORDS_PER_TITLE)

    fun generateLorem(): String = generateWords(MAX_WORDS_PER_LOREM)

    private fun generateWords(maxWords: Int): String {
        val wordCount = random.nextInt(maxWords - 1) + 1
